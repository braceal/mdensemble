...
def run_task(
    ...
    workdir = output_dir if node_local_path is None else node_local_path
    ...
    # Move from node local to persistent storage
    if node_local_path is not None:
        shutil.move(workdir, output_dir / workdir_name)


# TODO: In the future this can be replaced with parsl ParslPoolExecutor
class Thinker(BaseThinker):
    """Thinker that runs a single function on a list of input arguments."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.logger.info(
            f'Processing {len(self.input_arguments)} input arguments',
        )
    ...
    @result_processor(topic='task')
    def process_task_result(self, result: Result) -> None:
        """Handle the returned result of the task function and log status."""
...
class WorkflowSettings(BaseModel):
    ...
    output_dir: Path = Field(
        ...,
        description='Path this particular workflow writes to.',
    )

    # Simulation settings
    simulation_input_dir: Path = Field(
        ...,
        description='Directory with subdirectories each storing initial '
        ' simulation start files.',
    )

    simulation_config: MDSimulationSettings = Field(
        ...,
        description='Static simulation settings.',
    )

    # Compute settings
    num_parallel_tasks: int = Field(
        default=4,
        description='Number of parallel task to run '
        '(should be the total GPUs).',
    )
    node_local_path: Path | None = Field(
        default=None,
        description='Node local storage option for writing output files.',
    )
    ...
    def _validate_paths(self) -> Self:
        ...
        if not self.simulation_input_dir.exists():
            raise FileNotFoundError(str(self.simulation_input_dir))

        # Create the output directory if it does not exist
        self.output_dir.mkdir(exist_ok=True, parents=True)

        # Resolve the paths
        self.simulation_input_dir = self.simulation_input_dir.resolve()
...
if __name__ == '__main__':
    ...
    # Collect initial simulation directories, assumes they
    # are in nested subdirectories
